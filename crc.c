...
/*  C R C . C                                                               */
/*                                                                          */
/*  Das Programm berechnet einen 32 Bit CRC für eine Datei. Das Polynom     */
...
#define POLYNOM          0x04C11DB7U
#define POLYNOM_REVERSED 0xEDB88320U
#define crcPolynom       POLYNOM_REVERSED
...
/*----------------------------------------------------------------------------------------------
  ...
  void normalCrcTableBuild(unsigned int crctab[256]) {
    for (unsigned int i = 0; i < 256; i++) {
      unsigned int t = i;
      for (int j = 0; j < 8; j++)
        t = (t >> 1) ^ ((t & 1) ? crcPolynom : 0);

      crctab[i] = t;
    }
  }

  void fastCrcTableBuild(unsigned int crctab[256]) {
    unsigned int r;

    crctab[0] = 0;
    crctab[128] = r = crcPolynom;
    for (int i = 64; i; i /= 2)
      crctab[i] = r = (r >> 1) ^ (crcPolynom & ~((r & 1) - 1));

    for (int i = 2; i < 256; i *= 2)
      for (int j = 1; j < i; j++)
        crctab[i + j] = crctab[i] ^ crctab[j];
  }
----------------------------------------------------------------------------------------------*/

struct {
  int insertCRbeforeLF;
...
int main(int argc, char* argv[])
{
  ...
  if (argc >= 2)
    for (int i = 1; i < argc; i++)
      printCrcForNamedFile(argv[i]);
  else
    if (argc < 2 && !isatty(fileno(stdin)))
      printCrcForFile(useStdin(), "<stdin>");
    else
      usage();
  return 0;
}
...
void parseOptions(int* argcRef, char** argvRef[])
{
  ...
  for (; argc > 1 && argv[1][0] == '-'; argc--, argv++)
    for (int i = 1; argv[1][i]; i++)
      switch (argv[1][i])
      {
        case 'd': opt.insertCRbeforeLF = 1; break;
        case 't': opt.ignoreTrailingSpaces = 1; break;
      }
...
{
  for (unsigned char* bufferEnd = bufferPos + bufferLength; bufferPos < bufferEnd; bufferPos++)
  {
    if (*bufferPos == '\n')
    {
      if (opt.ignoreTrailingSpaces)
        crc.total = crc.noTrailingSpaces;
      if (opt.insertCRbeforeLF)
        crc.total = crctab[(unsigned char) crc.total ^ 0x0D] ^ (crc.total >> 8);
    }
    crc.total = crctab[(unsigned char) crc.total ^ *bufferPos] ^ (crc.total >> 8);
    if (*bufferPos != ' ')
      crc.noTrailingSpaces = crc.total;
  }
  return crc;
}
